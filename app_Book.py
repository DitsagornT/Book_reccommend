...
if st.button('Recommend Books'):
    if user_id_input in df_result_filter_missing_b_5_per['User-ID'].values:
        ...
        # ถ้า user ไม่มีในข้อมูล
        st.warning("User ID not found in the dataset.")
        st.write("ลองเลือกหนังสือที่คุณชอบ เพื่อรับคำแนะนำจากโมเดล")
        ...
        if st.button("Recommend Similar Books"):
            user_input_vector = np.full((1, input_dim), -1.0)  # ใช้ -1 แทน missing
